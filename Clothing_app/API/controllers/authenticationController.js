...
module.exports = {                  // mapping data from req to the new user
    newUser: async (req, res) => {const user = new UserRelation({
    ...
    try {           // user creation check
        await user.save();
        res.status(201).json({messege: "New user has been created"})
    } catch (err) {
        console.log(err )
    ...
    login: async (req, res) => {
        try {
            ...
            !user && res.status(401).json({messege: "There is no account connected to this email address"});
            ...
            passDecrypt !== req.body.password && res.status(401).json({messege: "Password incorrect, try again"});      // check for password matches the password from the database
